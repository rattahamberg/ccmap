...
describe('Registry', () => {
  ...
  it('update modifies an existing feature', () => {
    ...
    const feature = makeFeature();
    registry.add(feature);
    registry.update('test-1', { type: 'water' });
    const updated = registry.get('test-1');
    expect(updated?.type).toBe('water');
  ...
  it('update does nothing for non-existent id', () => {
    const registry = createRegistry();
    registry.update('nonexistent', { type: 'water' });
  ...
  it('add with duplicate ID overwrites existing feature silently', () => {
    const registry = createRegistry();
    const original = makeFeature({ id: 'dup' });
    const replacement = makeFeature({ id: 'dup', type: 'border', closed: false });

    registry.add(original);
    registry.add(replacement);

    expect(registry.getAll()).toHaveLength(1);
    expect(registry.get('dup')).toEqual(replacement);
  });
